...
function getInicioDiferencia(act) {
    ...
    if (act.es_todo_el_dia == 'Sí') {
        ...
        try {
            ...
            throw new Error(`Error while parsing tiempo_inicio: ${error}`, {cause: error});
        }
    }
...
export function fechaFromActividadRecurrencia(act, rec) {
...
export function getLastEventOfRecurrencia(act, rec) {
    let inicio = primeraRecurrencia(act, rec);
...
export function getLastEventOfActividad(act, rec_list) {
...
export function getFinActividad(act_instance, t_fin) {
