...
async function parseFusiones(filename) {
    ...
    .on('error', (error) => uiLogger.error(`Error while trying to read file ${filename}: ${error}`));
...
async function parseHorarios(filename) {
    ...
    .on('end', async () => {
        ...
        for (let i = 0; i < results.length; i++) {
            ...
            try {
                ...
                uiLogger.error(`Something went wrong while parsing: ${error}`);
    ...
    uiLogger.info('Horarios insertados');
}
