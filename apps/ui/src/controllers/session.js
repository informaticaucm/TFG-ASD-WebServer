...
export async function createUser(req, res) {
  ...
  try {
    await sendToApiJSON(resultado, '/usuarios', res, false);
...
export async function assignMAC(req, res) {
...
export async function assignNFC(req, res) {
  ...
  for (uid in req.body) {
    if (req.body[uid] != null && nfc_regex.test(req.body[uid])) {
      try {
        await sendToApiJSON({ uid: req.body[uid] }, `/usuarios/nfcs/${req.session.user.id}`, res, true);
      }
      catch (error) {
        uiLogger.error(error);
        throw error;
