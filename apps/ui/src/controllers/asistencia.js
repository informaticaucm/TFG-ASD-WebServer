...
export async function justificar(req, res) {
    ...
    try {
        ...
        await sendToApiJSON(data, `/seguimiento/asistencias/${id}`, res, false);
...
export async function filtrarAsistencias(req, res) {
    ...
    const espacios_ids = await getFromApi('/espacios', res, true);
    let espacios = [];
    for (let i  = 0; i < espacios_ids.length; i++) {
        const espacio_info = await getFromApi(`/espacios/${espacios_ids[i].id}`, res, true);
        espacios.push({id: espacios_ids[i].id, nombre: espacio_info.nombre, seleccionado: data.espacioId == espacios_ids[i].id});
    }

    const docentes_ids = await getFromApi('/usuarios', res, true);
    let docentes = [];
    req.session.user.sustituto_ids = {};
    for (let i  = 0; i < docentes_ids.length; i++) {
        const docente_info = await getFromApi(`/usuarios/${docentes_ids[i].id}`, res, true);
...
export async function confirmarFirma(req, res) {
    ...
    if (req.body.sustituto != 'no') {
        ...
        const actividad = await getFromApi(`/actividades/${ids.actividad_id}`, res, true);
        let mmt_inicio = moment(actividad.tiempo_inicio, "HH:mm");
...
export async function generarAvisos(req, res) {
...
export async function verProfesoresInfracciones(req, res) {
    ...
    for (let i = 0; i < asistencias_ids.length; i++) {
        ...
        if (profesores[docente_id].nombre == null) {
            const docente_info = await getFromApi(`/usuarios/${docente_id}`, res, true);
