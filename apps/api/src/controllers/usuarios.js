...
export async function authenticateUser(req, res, next, db) {
    const transaction = await db.sequelize.transaction();
    
    try {
        apiLogger.info('Searching in Docente for id, email, password, nombre, apellidos');
...
export async function createUser(req, res, next, db) {
    ...
    try {
        ...
        if (!nuevo) {
            await transaction.rollback();
            apiLogger.error(`El docente con el email ${req.body.email} ya existe en la base de datos`);
            let err = {}
            err.status = 409;
...
export async function getUsuarios(req, res, next, db) {
    const transaction = await db.sequelize.transaction();
...
export async function getUsuarioById(req, res, next, db) {
    ...
    try {
        apiLogger.info('Searching in Docentes for id, nombre, apellidos, email');
...
export async function registerMACToUsuario(req, res, next, db) {
    let idUsuario = Number(req.params.idUsuario);
    if (!Number.isInteger(idUsuario)) {           
        let err = {};
        err.status = 400;
...
export async function registerNFCToUsuario(req, res, next, db) {
    let idUsuario = Number(req.params.idUsuario);
    if (!Number.isInteger(idUsuario)) {           
        let err = {};
        err.status = 400;
