...
import { attendanceLogger } from '../config/attendance-logger.js';
...
export async function registroAsistencia(req, res, next, db) {

    apiLogger.info(`Asistencia a registrar con datos: ${JSON.stringify(req.body)}`);
    ...
    if (Object.keys(req.body).length > 0 && req.body.tipo_registro != null && req.body.espacioId != null && Number.isInteger(req.body.espacioId)) {
        ...
        apiLogger.info('Searching in Espacio for id');
        ...
        switch (req.body.tipo_registro) {
            case "RegistroSeguimientoFormulario": // Método de registro para administración (registrar firmas)
                if (req.body.usuarioId != null && Number.isInteger(req.body.usuarioId) && req.body.estado != null && valoresAsistencia.includes(req.body.estado)) {
                    ...
                    try {
                        ...
                        attendanceLogger.info('Registrada asistencia sin totp de ' + query_user.dataValues.nombre + ' ' + query_user.dataValues.apellidos + 
            ...
            case "RegistroSeguimientoUsuario": // Método registro a través del formulario (es necesario el totp)
                if (req.body.usuarioId != null && Number.isInteger(req.body.usuarioId)) {
                    ...
                    try {
                        apiLogger.info('Searching in Dispositivo for id');
                        ...
                        await transaction.commit();
                        attendanceLogger.info('Registrada asistencia con totp válido de ' + query_user.dataValues.nombre + ' ' + query_user.dataValues.apellidos + 
                                ' (' + query_user.dataValues.id + ') en ' + query_esp.dataValues.tipo + ' ' + query_esp.dataValues.numero + ' ' + 
                                query_esp.dataValues.edificio + ' (' + query_esp.dataValues.id + ') con estado ' + req.body.estado);
            ...
            case "RegistroSeguimientoDispositivoBle": // caso de registro por bluetooth
                if (req.body.mac != null && req.body.dispositivoId != null && Number.isInteger(req.body.dispositivoId)) {
                    ...
                    try {
                        ...
                        apiLogger.info(`Docente de la MAC ${query_user}`);
                        ...
                        await transaction.commit();
                        attendanceLogger.info('Registrada asistencia por bluetooth de ' + query_user.dataValues.nombre + ' ' + query_user.dataValues.apellidos + 
                                ' (' + query_user.dataValues.id + ') en ' + query_esp.dataValues.tipo + ' ' + query_esp.dataValues.numero + ' ' + 
            ...
            case "RegistroSeguimientoDispositivoNFC": // caso de registro por lectura de nfc
                if (req.body.uid != null) {
                    ...
                    try {
                        ...
                        await transaction.commit();
                        attendanceLogger.info('Registrada asistencia por NFC de ' + query_user.dataValues.nombre + ' ' + query_user.dataValues.apellidos + 
                                ' (' + query_user.dataValues.id + ') en ' + query_esp.dataValues.tipo + ' ' + query_esp.dataValues.numero + ' ' + 
                                query_esp.dataValues.edificio + ' (' + query_esp.dataValues.id + ') con estado ' + (checkEstado) ? valoresAsistencia[0] : valoresAsistencia[1]);
                    }
                    catch (error) {
...
export async function getMacsBLE(req, res, next, db) {
    ...
    if (Object.keys(req.body).length > 0 && req.body.espacioId != null) {
        ...
        try {
            ...
            apiLogger.info('Searching in Actividad for id');
...
export async function getAsistenciaById(req, res, next, db) {
    ...
    try {
        apiLogger.info('Searching in Asistencia for id, docente_id, espacio_id, fecha, estado, motivo');
...
export async function updateAsistenciaById(req, res, next, db) {
...
export async function registroPLA(req, res, next, db) {
    ...
    .on('error', (error) => apiLogger.error(`Error while trying to read file ${pla_logs}: ${error}`))
    .pipe(csv({separator: ';'}))
...
export async function checkEstadoAsistencia(db, docenteId, espacioId, fecha) {
